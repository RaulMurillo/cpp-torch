...
// #define CHECK_CUDA(x) AT_ASSERT(x.is_contiguous(), #x " must be contiguous")
...
std::vector<torch::Tensor> conv_backward(torch::Tensor input,
    ...
    std::vector<torch::Tensor> gradParams = backward_gradParameters(input, gradOutput, weights, kW, kH, dW, dH, padW, padH, is_bias);
